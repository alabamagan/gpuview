...
def all_gpustats():
    ...
    for url in hosts:
        try:
            # check if port is specified, if not add default port 9988
            if re.match(':[0-9]+', url) is None:
                url = url.replace(':', '') # remove ':' first to prevent error if there are more than one
                url = url + ':9988'
            raw_resp = urlopen('http://' + url + '/gpustat')
